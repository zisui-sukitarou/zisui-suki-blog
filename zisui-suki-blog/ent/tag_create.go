...
func (tc *TagCreate) SetStatus(u uint) *TagCreate {
...
func (tc *TagCreate) check() error {
	if _, ok := tc.mutation.Icon(); !ok {
		return &ValidationError{Name: "icon", err: errors.New(`ent: missing required field "Tag.icon"`)}
	}
	if _, ok := tc.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`ent: missing required field "Tag.status"`)}
	}
