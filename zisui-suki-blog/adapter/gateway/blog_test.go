...
func findByUserId(db *db.DB, userId model.UserId, begin uint, end uint) ([]*repository.BlogData, error) {
...
func TestFindByUserId(t *testing.T) {
	...
	for _, d := range data {
		...
		for i, blog := range blogs {
			log.Println("blog", i, ":")
			log.Println("- blog_id:", blog.Blog.BlogId)
			log.Println("- abstract:", blog.Blog.Abstract)
			log.Println("- writer:", blog.Blog.UserId)
			log.Println("- tags:")
			for it, tag := range blog.Tags {
				log.Println(" -", it, ":", tag.TagName)
			}
		}
	}
}
...
func findByTagName(db *db.DB, tagName model.TagName, begin uint, end uint) ([]*repository.BlogData, error) {
...
func findByUserIdAndTagName(db *db.DB, userId model.UserId, tagName model.TagName, begin uint, end uint) ([]*repository.BlogData, error) {
