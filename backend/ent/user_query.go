...
// QueryFavors chains the current query on the "favors" edge.
func (uq *UserQuery) QueryFavors() *BlogQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withFavors: uq.withFavors.Clone(),
...
// WithFavors tells the query-builder to eager-load the nodes that are connected to
// the "favors" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithFavors(opts ...func(*BlogQuery)) *UserQuery {
	query := &BlogQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFavors; query != nil {
		if err := uq.loadFavors(ctx, query, nodes,
			func(n *User) { n.Edges.Favors = []*Blog{} },
			func(n *User, e *Blog) { n.Edges.Favors = append(n.Edges.Favors, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadFavors(ctx context.Context, query *BlogQuery, nodes []*User, init func(*User), assign func(*User, *Blog)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.FavorsTable)
		s.Join(joinT).On(s.C(blog.FieldID), joinT.C(user.FavorsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.FavorsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.FavorsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "favors" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
